...
- [A-08][Agent A] `GOOGLE_API_KEY` và SDK đã sẵn sàng; schema Gemini đã tương thích. Nhưng `infra.llm.DEFAULT_MODEL = "gemini-1.5-flash"` trả `NotFound`; thử `gemini-3.5-flash` cũng `NotFound`. Cần Agent C cấu hình model Gemini khả dụng trong `infra/` rồi Agent A chạy lại 8 mẫu thật.
