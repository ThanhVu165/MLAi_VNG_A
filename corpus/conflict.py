...
STOP_WORDS = frozenset({"các", "cho", "của", "được", "học", "khi", "là", "này", "theo", "trong", "và"})
...
def flag_active_conflicts(
    *, actor: str,
    ...
    chunks = [chunk for chunk in list_chunks(database_path=database_path) if chunk.doc_id in active_ids]
    pairs: list[ConflictPair] = []
    for first, second in combinations(chunks, 2):
        if first.doc_id == second.doc_id or not _conflicts(first, second):
            continue
        update_chunk(replace(first, conflict_flag=True, conflict_with=second.chunk_id), database_path=database_path)
        update_chunk(replace(second, conflict_flag=True, conflict_with=first.chunk_id), database_path=database_path)
