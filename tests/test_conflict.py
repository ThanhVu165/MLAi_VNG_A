...
def test_conflicting_active_sources_are_flagged_and_supersede_is_scheduled(tmp_path: Path) -> None:
    ...
    first = ChunkRecord(
        "chunk-3", source_three.doc_id, "Điều 1", "Hạn chót rút học phần là 15/09/2026.", Domain.COURSE_WITHDRAWAL
    )
    second = ChunkRecord(
        "chunk-5", source_five.doc_id, "Điều 2", "Hạn chót rút học phần là 20/09/2026.", Domain.COURSE_WITHDRAWAL
    )
    ...
    assert [(pair.first_chunk_id, pair.second_chunk_id) for pair in pairs] == [("chunk-3", "chunk-5")]
